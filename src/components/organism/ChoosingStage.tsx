...
const ChoosingState = ({
  ...
  const handleSelectFactory = (factory: Factory) => {
    ...
    // czy w tym momencie mgoe odmontowac component?
  };
  ...
  return (
    <div className="border-[3px] w-full sm:w- shadow-[5px_6px_0px] bg-amber-50 rounded-3xl p-4">
      ...
      {step == 1 && (
        <SelectRegion state={state} handleSelect={handleSelectRegion} />
      )}
      {step == 2 && (
        <SelectFactory state={state} handleSelect={handleSelectFactory} />
      )}
    </div>
  );
};
